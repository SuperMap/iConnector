 ...
 * @constructor
 */
SuperMap.Web.Adapter.LeafletAdapter = function(){
 ...
 */
SuperMap.Web.Adapter.LeafletAdapter.getLayer = function(url,options){
    if(url == undefined)
    {
        return;
    }
 ...
 */
SuperMap.Web.Adapter.LeafletAdapter.transferPoint = function(array,projection){
    if((typeof array) == "object" && array != null && array.constructor == Array)
    {
        ...
        for(var i = 0;i<array.length;i++)
        {
            ...
            var traPoint = SuperMap.Web.Adapter.LeafletAdapter.transfer(smPoint.x,smPoint.y);
 ...
 */
SuperMap.Web.Adapter.LeafletAdapter.transferLine = function(array,projection){
    if((typeof array) == "object" && array != null && array.constructor == Array)
    {
        var pro = projection || new SuperMap.Projection("EPSG:4326");
        var lines = [];
        ...
        for(var i = 0;i<array.length;i++)
        {
            ...
            {
                var points = SuperMap.Web.Adapter.LeafletAdapter.transferPoint(array[i].components,pro);
                line = new L.Polyline(points);
            }
            //支持L.Polyline的对象
            else if(array[i].constructor == L.Polyline)
            {
                var points = SuperMap.Web.Adapter.LeafletAdapter.transferPoint(array[i].getLatLngs(),pro);
                line = new L.Polyline(points);
            }
 ...
 */
SuperMap.Web.Adapter.LeafletAdapter.transferPolygon = function(array,projection){
    if((typeof array) == "object" && array != null && array.constructor == Array)
    {
        var pro = projection || new SuperMap.Projection("EPSG:4326");
        var polygons = [];
        ...
        for(var i = 0;i<array.length;i++)
        {
            ...
            {
                var points = SuperMap.Web.Adapter.LeafletAdapter.transferPoint(array[i].getVertices(false),pro);
 ...
 */
SuperMap.Web.Adapter.LeafletAdapter.transfer = function(lng,lat){
