...
/**
 * Class: SuperMap.Web.iConnector.Baidu
 * 百度适配器类
 */
...
/**
 * Property:
 * {number} 记录向百度服务器点坐标转换请求次数，用于服务器处理完成后定位回调函数,每请求一次服务器，会自动累加。
 */
 ...
 *
 * Parameters:
 * points - {Array}百度点BMap.Point数组
 * type - {Number} 0代表GPS坐标转百度坐标；2代表google坐标转百度坐标
 * id - {String} 唯一标示第几批点数组，对应了
 */
 ...
 *
 * Returns:
 * {BMap.TileLayer} 返回百度的BMap.TileLayer对象
 */
 ...
 * projection - {SuperMap.Projection} 待转换点的投影系（数组里面的所有点投影系都必须是统一的）
 * callback - {Function} 所绑定的回调函数  （回调函数会以数组形式返回转换后的点数组）
 * type - {Number} 首先数据会在内部转换为4326坐标系，当为0时代表数据是标准GPS坐标转向百度坐标，当为2时代表数据是根据国家测绘局规定进行了统一偏移的数据（中国范围内的google数据就是其中之一）转向百度坐标，默认为0.
 *
 * Examples:
 * (start code)
 * var points = [
 ...
 *  callback - {Function} 所绑定的回调函数（回调函数会以数组形式返回转换后的线数组）
 *  type - {Number} 首先数据会在内部转换为4326坐标系，当为0时代表数据是标准GPS坐标转向百度坐标，当为2时代表数据是根据国家测绘局规定进行了统一偏移的数据（中国范围内的google数据就是其中之一）转向百度坐标，默认为0.
 *
 *  Examples:
 *  (start code)
 ...
 *
 *  Parameters:
 *  polygons - {Array} 面数组，支持两种形式
 *  projection - {SuperMap.Projection} 需要转换的面的坐标系
 *  callback - {Function} 所绑定的回调函数（回调函数会以数组形式返回转换后的面数组）
 *  type - {Number} 首先数据会在内部转换为4326坐标系，当为0时代表数据是标准GPS坐标转向百度坐标，当为2时代表数据是根据国家测绘局规定进行了统一偏移的数据（中国范围内的google数据就是其中之一）转向百度坐标，默认为0.
 *
 *  Examples:
 *  (start code)
...
/**
 * Property: startPointArray
 * {Array} BMap.Point数组的数组，记录转换前的点数组，首先本身是一个数组，每一个数据代表某一批点数组，每批点数组又是多个点组成的
 */
SuperMap.Web.iConnector.Baidu.startPointArray = [];
/**
 * Property: endPointArray
 * {Array} BMap.Point数组的数组，记录转成后的点数组的数组，首先本身是一个数组，每一个数据代表某一批点数组，每批点数组又是多个点组成的
 */
SuperMap.Web.iConnector.Baidu.endPointArray = [];
/**
 * Property: callbackPointEvent
 * {Array} 回调函数数组，记录用户注册的点回调函数数组
 */
SuperMap.Web.iConnector.Baidu.callbackPointEvent = [];
/**
 * Property: callbackPointEventCounts
 * {number} 记录当前为第多少批点数组需要进行转换，默认为-1，每有一批点数组需要转换就自加1。
 */
SuperMap.Web.iConnector.Baidu.callbackPointEventCounts = -1;
/**
 * Method: circulatePointSend
 * 每次服务器转换完点后的回调函数，在此判定是否将所有点全部转换，如果没有则继续转换
 *
 * Parameters: circulatePointSend
 * xyResults - {Array} 服务器传回的坐标集合
 * id - {String} 代表此次转换完的点是属于第id批的点数组，避免回调函数出错
 * type - {Number} 首先数据会在内部转换为4326坐标系，当为0时代表数据是标准GPS坐标转向百度坐标，当为2时代表数据是根据国家测绘局规定进行了统一偏移的数据（中国范围内的google数据就是其中之一）转向百度坐标，默认为0.
 */
...
/**
 *  Property: startLineArray
 *  {Array}线数组的数组，记录转换前的线数组的数组，首先本身是一个数组，每一个数据代表某一批线数组，每批线数组又是多条线组成的，每一条线其实又是点数组。
 */
SuperMap.Web.iConnector.Baidu.startLineArray = [];
/**
 * Property: endLineArray
 * {Array}  BMap.Polyline数组的数组，记录转换后的线数组的数组，首先本身是一个数组，每一个数据代表某一批BMap.Polyline线数组，每批线数组又是多条BMap.Polyline线组成的。
 */
SuperMap.Web.iConnector.Baidu.endLineArray = [];
/**
 * Property: callbackLineEvent
 * {Array} 回调函数数组，记录用户注册的线回调函数数组。
 */
SuperMap.Web.iConnector.Baidu.callbackLineEvent = [];
/**
 * Property: callbackLineEventCounts
 *  {number}  默认为-1，每有一批线数组需要转换就自加1，记录当前为第多少批线数组需要进行转换。
 */
SuperMap.Web.iConnector.Baidu.callbackLineEventCounts = -1;
/**
 * Method: circulateLineSend
 * 每次服务器转换完线后的回调函数，在此判定是否将所有线全部转换，如果没有则继续转换。
 *
 * Parameters:
 * points {Array}  转换后的每一条线的点集合
 * id - {String} 代表此次转换完的线是属于第id批的线数组，避免回调函数出错
 * type - {Number} 首先数据会在内部转换为4326坐标系，当为0时代表数据是标准GPS坐标转向百度坐标，当为2时代表数据是根据国家测绘局规定进行了统一偏移的数据（中国范围内的google数据就是其中之一）转向百度坐标，默认为0.
 */
...
/**
 *  Property: startPolygonArray
 *  {Array} 面数组的数组，记录转换前的面数组的数组，首先本身是一个数组，每一个数据代表某一批面数组，每批面数组又是多个面组成的，每一个面其实又是点数组组成的。
 */
SuperMap.Web.iConnector.Baidu.startPolygonArray = [];
/**
 * Property: endPolygonArray
 * {Array}  BMap.Polygon数组的数组，记录转换后的面数组的数组，首先本身是一个数组，每一个数据代表某一批BMap.Polygon面数组，每批面数组又是多个BMap.Polygon面组成的
 */
SuperMap.Web.iConnector.Baidu.endPolygonArray = [];
/**
 * Property: callbackPolygonEvent
 * {Array}  回调函数数组，记录用户注册的面回调函数数组
 */
SuperMap.Web.iConnector.Baidu.callbackPolygonEvent = [];
/**
 * Property: callbackPolygonEventCounts
 *  {number} 记录当前为第多少批面数组需要进行转换，默认为-1，每有一批面数组需要转换就自加1
 */
SuperMap.Web.iConnector.Baidu.callbackPolygonEventCounts = -1;
/**
 * Method: circulatePolygonSend
 * 每次服务器转换完面后的回调函数，在此判定是否将所有面全部转换，如果没有则继续转换
 *
 * Parameters:
 * points - {Array}  转换后的每一个面的点集合
 * id - {String} 代表此次转换完的面是属于第id批的面数组，避免回调函数出错
 * type - {Number} 首先数据会在内部转换为4326坐标系，当为0时代表数据是标准GPS坐标转向百度坐标，当为2时代表数据是根据国家测绘局规定进行了统一偏移的数据（中国范围内的google数据就是其中之一）转向百度坐标，默认为0.
 */
